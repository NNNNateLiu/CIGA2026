...
namespace UniversalWaterSystem
{
    ...
    {
        static public RenderTextureFormat FourChannelFormat = RenderTextureFormat.ARGBHalf;
        static public RenderTextureFormat TwoChannelFormat = RenderTextureFormat.RGHalf;
                            ...
                            Texture2D gaussianNoise)
        {
            ...
            initialSpectrum = FFTCompute.CreateRenderTexture(size, FourChannelFormat);
            precomputedData = FFTCompute.CreateRenderTexture(size, FourChannelFormat);
            displacement = FFTCompute.CreateRenderTexture(size, FourChannelFormat);
            derivatives = FFTCompute.CreateRenderTexture(size, FourChannelFormat, true);
            turbulence = FFTCompute.CreateRenderTexture(size, FourChannelFormat, true);
            
            paramsBuffer = new ComputeBuffer(2, 8 * sizeof(float));
            
            buffer = FFTCompute.CreateRenderTexture(size, TwoChannelFormat);
            DxDz = FFTCompute.CreateRenderTexture(size, TwoChannelFormat);
            DyDxz = FFTCompute.CreateRenderTexture(size, TwoChannelFormat);
            DyxDyz = FFTCompute.CreateRenderTexture(size, TwoChannelFormat);
            DxxDzz = FFTCompute.CreateRenderTexture(size, TwoChannelFormat);
        }
